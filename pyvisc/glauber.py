...
class Glauber(object):
    ...
    def __init__(self, cfg, ctx, queue, compile_options, d_ev1, save_nbc=False):
        ...
        self.kernel_glauber.glauber_ini(queue, (cfg.NX, cfg.NY), None, d_ev1).wait()
        if save_nbc: self.save_nbinary(ctx, queue, cfg)
    ...
    def __loadAndBuildCLPrg(self, ctx, cfg):
        ...
        glauber_defines.append('-D {key}={value:f}f'.format(key='Eta', value=cfg.Eta))
        glauber_defines.append('-D {key}={value:f}f'.format(key='Si0', value=cfg.Si0))
        glauber_defines.append('-D {key}={value:f}f'.format(key='ImpactParameter', value=cfg.ImpactParameter))
        glauber_defines.append('-D {key}={value:f}f'.format(key='Edmax', value=cfg.Edmax))
        glauber_defines.append('-D {key}={value:f}f'.format(key='Hwn', value=cfg.Hwn))
        glauber_defines.append('-D {key}={value:f}f'.format(key='Eta_flat', value=cfg.Eta_flat))
        glauber_defines.append('-D {key}={value:f}f'.format(key='Eta_gw', value=cfg.Eta_gw))
