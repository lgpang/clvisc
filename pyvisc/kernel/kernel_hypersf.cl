...
// calc the vorticity omega_{xz}=dxu_z - dz u_x on freeze out hypersf
__kernel void vorticity_hypersf(__global real  * d_vorticity,
                          __global int * num_of_sf,
                          __global real4 * d_ev_old,
                          __global real4 * d_ev_new,
                          const real time_old,
    ...
    __private real4 ev_cube[16];
    __private real ed_cube[16];
    __private real vorticity_cube[16];
    int i = get_global_id(0);
    int j = get_global_id(1);
    int k = get_global_id(2);
    ...
    if ( (i+1)*nxskip < NX && (j+1)*nyskip < NY && (k+1)*nzskip < NZ ) {
        ...
        if ( is_surf ) {
            ...
            int J=j;
            int K=k;
            vorticity_cube[0] = omega_xz(
                    d_ev_old[idn(I, J, K)],   d_ev_new[idn(I, J, K)],
                    d_ev_old[idn(I-1, J, K)], d_ev_old[idn(I+1, J, K)],
                    d_ev_old[idn(I, J, K-1)], d_ev_old[idn(I, J, K+1)],
                    time_old, -NZ/2*DZ + K*dzd, dtd, dxd, dzd);

            I = i + 1; J = j; K = k;
            vorticity_cube[1] = omega_xz(
                    d_ev_old[idn(I, J, K)],   d_ev_new[idn(I, J, K)],
                    d_ev_old[idn(I-1, J, K)], d_ev_old[idn(I+1, J, K)],
                    d_ev_old[idn(I, J, K-1)], d_ev_old[idn(I, J, K+1)],
                    time_old, -NZ/2*DZ + K*dzd, dtd, dxd, dzd);
            ...
            I = i; J = j; K = k;
            vorticity_cube[8] = omega_xz(
                    d_ev_old[idn(I, J, K)],   d_ev_new[idn(I, J, K)],
                    d_ev_new[idn(I-1, J, K)], d_ev_new[idn(I+1, J, K)],
                    d_ev_new[idn(I, J, K-1)], d_ev_new[idn(I, J, K+1)],
                    time_new, -NZ/2*DZ + K*dzd, dtd, dxd, dzd);

            I = i + 1; J = j; K = k;
            vorticity_cube[9] = omega_xz(
                    d_ev_old[idn(I, J, K)],   d_ev_new[idn(I, J, K)],
                    d_ev_new[idn(I-1, J, K)], d_ev_new[idn(I+1, J, K)],
                    d_ev_new[idn(I, J, K-1)], d_ev_new[idn(I, J, K+1)],
                    time_new, -NZ/2*DZ + K*dzd, dtd, dxd, dzd);
            ...
            //real4 ev = centroid_ev(ev_cube, mass_center);
            real vorticity_xz = centroid_omegaxz(vorticity_cube, mass_center);
                                  
            d_vorticity[atomic_inc(num_of_sf)] = vorticity_xz;
        } // end surface calculation
    } // end boundary check
}
