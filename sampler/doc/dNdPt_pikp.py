...
def get_dNdPt(dat0, pid=211):
    ...
    return pt, dN/(2*np.pi*float(NEvent)*dpt*1.0)




if __name__=='__main__':
    path = '/Users/lgpang/Documents/D0/data/D0_PCE_0p20/'
    fname= path + 'mc_particle_list.dat'
    ...
    dndpt0 = np.loadtxt('%s/dNdYPtdPt_over_2pi_211.dat'%path)
    dndpt1 = np.loadtxt('%s/dNdYPtdPt_over_2pi_321.dat'%path)
    dndpt2 = np.loadtxt('%s/dNdYPtdPt_over_2pi_2212.dat'%path)
    ax.semilogy(dndpt0[:, 0], dndpt0[:,0]*dndpt0[:, 1], 'r-', label='smooth')
    ...
    ax.set_ylabel(r'$\frac{dN}{2\pi dY dp_T}\ [GeV]^{-1}\ for\ \pi^+$')
